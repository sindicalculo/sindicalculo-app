...
import { rescisaoSchema } from './schema';
import { RescisionCltCalculator } from './engine';
import { TerminationResult } from './types';
import { SindicatoData } from '@/modules/sindicatos/types';
import { salvarAssociado } from '@/modules/associados/actions';

export async function processarRescisao(formData: unknown): Promise<{ success?: boolean; data?: TerminationResult; sindicato?: SindicatoData; error?: string }> {
  try {
    const validatedData = rescisaoSchema.parse(formData);
    ...
    const result = RescisionCltCalculator.calculate(validatedData, sindicatoData?.cctConfig);
    ...
    return { error: error.message || "Erro de validação ou processamento da rescisão." };
  }
}
